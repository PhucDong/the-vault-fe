...
function AiringStatusFilter(props) {
  const { airingStatusOption, setAiringStatusOption, sx } = props;

  const handleChangeAiringStatusFilter = (event) =>
    setAiringStatusOption(event.target.value);
  ...
  return (
    <Box sx={sx}>
      ...
      <TextField
        ...
        select
        hiddenLabel
        value={airingStatusOption}
        onChange={handleChangeAiringStatusFilter}
        slotProps={{
          select: {
            multiple: false,
      ...
      >
        {airingStatusOptionList.map((option) => (
          <MenuItem key={option} value={option}>
            <Checkbox
              id={option}
              name={option}
              checked={airingStatusOption.indexOf(option) > -1}
            />
...
export default AiringStatusFilter;
