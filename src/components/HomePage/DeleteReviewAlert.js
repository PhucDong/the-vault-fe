...
function DeleteReviewAlert(props) {
  const {
    reviewId,
    commentId,
    openDeleteAlert,
  ...
  const handleDeleteUserReview = async () => {
    try {
      ...
      if (location.pathname === "/home") {
        reviewListData = await apiService.get("/reviews");
        setReviewList(reviewListData.reviewList);
      } else if (location.pathname === "/home/me") {
        reviewListData = await apiService.get("/users/me");
        setReviewList(reviewListData.user.reviews);
      }
    } catch (error) {
      console.log(error);
    }
